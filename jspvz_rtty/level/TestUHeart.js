oS.Init({
	PName: [oPeashooter, oPotatoMine, oSquash, oCherryBomb, oJalapeno, oTallNut],
	ZName: [oZombie],
	...
	StartGame: function() {
		...
		innerText($("btnClick3"), "5000");
		innerText($("btnClick4"), "翠花,上50000个!!");
		CustomPlants(0, 2, 5);
		CustomPlants(0, 3, 9);
		CustomPlants(0, 4, 1);
		for (let i = 1; i <= 5; ++i) CustomPlants(5, i, 6), oGd.$[i + "_6_1"].HP *= 100000;
		oP.Monitor({
			ar: [0],
			f: function() {
				var c = $User.Browser,
				...
				$("btnClick3").onclick = function() {
					...
					innerText($("DivTeach"), "下面有请我们的5000个僵尸客串演员出场！");
					b()
				};
				$("btnClick4").onclick = function() {
					oP.FlagToSumNum.a2 = [50000];
					innerText($("DivTeach"), "有请50000个客串演员出场！！或许他们化妆需要一点时间，请耐心等待。。。");
					b()
				}; 
