    ...
    oSym.addTask(1, function(C, h, b) {
      ...
      oSym.addTask(5, arguments.callee, [C, h, b])
		...
		ChangeR: function(e) {
			...
			g.length ? (l = !d.WalkDirection ? -5 : 20, d.ZX += l, d.AttackedLX += l, d.AttackedRX += l, d.X += l, q = g[Math.floor(Math.random() * g.length)], SetStyle(f, {
				left: d.X + "px",
...
oDancingZombie= InheritO(OrnNoneZombies, {
	...
	ChkActs: function(g, d, h, c) {
		...
		if(g.HPlook&&!g.bHP){
	g.bHP=1;
    var E= NewEle("daHP", "div", "position:absolute;color:#fff;top:" + (g.pixelTop + 60) + "px;left:" + (g.ZX - 50) + "px;width:100%;font-size:12px", "", EDAll);
    var A = "shp" + Math.random();
    daHP.id = A;
    var C = $(A);
    E.innerHTML = '<div>' + (g.OrnHP + g.HP) + "</div>";
    oSym.addTask(1, function(C, g, E) {
      ClearChild(C);
      if (g.HP > g.BreakPoint && $Z[g.id]) {
        EDAll && EDAll.appendChild(C);
        C.style.left = (g.ZX - 50) + "px";
        E.innerHTML = '<div>' + (g.OrnHP + g.HP) + "</div>";
      }
...
oDancingZombie1= InheritO(OrnNoneZombies, {
	...
	Produce: '舞王僵尸和人类(在世或者死去的)如有雷同，纯属巧合。</p><p>韧性：<font color="#FF0000">中（1500）</font><br>特点：<font color="#FF0000">召唤小丑或辣椒爆炸</font></p>舞王僵尸辞退了原来的伴舞，并将路障收为伴舞，虽然路障们无法配合他的舞蹈，不过brains are the most important!</font></p>游戏内音乐：《Beat it》-Michael Jackson',
	getSnowPea: function() {
		this.PlaySlowballAudio();
	},
	NormalDie: function() {
	...
	PrivateAct: function(a){
		...
		ChkActs: function(e, j, q, k) {
			...
			GetC(e.AttackedLX) > 5 && (e.OSpeed = (e.Speed+= 0.01));
				if(e.HPlook&&!e.bHP){
	e.bHP=1;
    var E= NewEle("edHP", "div", "position:absolute;color:#fff;top:" + (e.pixelTop + 60) + "px;left:" + (e.ZX - 50) + "px;width:100%;font-size:12px", "", EDAll);
...
oBalloonZombie = InheritO(OrnIZombies, {
	...
	if(f.HPlook&&!f.bHP){
	f.bHP=1;
    var E = NewEle("dHP", "div", "position:absolute;color:#fff;top:" + (f.pixelTop + 60) + "px;left:" + (f.ZX - 50) + "px;width:100%;font-size:12px", "", EDAll);
    var A = "hp" + Math.random();
    dHP.id = A;
    var C = $(A);
    E.innerHTML = '<div>' + (f.OrnHP + f.HP) + "</div>";
