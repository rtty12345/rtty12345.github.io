...
oZombie2 = InheritO(oZombie, {
	...
	PrivateBirth: function(a){
            ...
            let pea = NewImg(z.PeaHead,"images/Plants/Cob.gif","position:absolute;width:80px;height:80px;transform:rotateY(180deg);left:45px;top:15px;",0);
            z.appendChild(pea);
        },
            ...
            oSym.addTask(1,function(z,d,a){
                ...
                for(let i = 3;i >= 0;i--){
                    for(let j = 1;j <= C;j++){
                        ...
                        p && (p.canEat) && (p.EName != "oLawnCleaner" && p.EName != "oPoolCleaner" && p.EName != "oBrains" && p.EName != "oPuffShroom" && p.EName != "oSunShroom" && p.EName != "oPotatoMine" && p.EName != "oCherryBomb" && p.EName != "oJalapeno" && p.EName != "oDoomShroom"&& p.EName != "oSunFlower") && (($(p.id).offsetLeft + $(p.id).offsetWidth >= $(d).offsetLeft) && ($(p.id).offsetLeft >= $(d).offsetLeft + $(d).offsetWidth)) && (PlayAudio("splat1"),(p.HP -=1800),($(d) && ClearChild($(d))));
