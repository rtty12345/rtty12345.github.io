	...
	if(!h.Ornaments){
      ...
      oSym.addTask(5, arguments.callee, [C, h, b])
		...
		ChkActs1: function(g, e, h, d) {
			...
			this.same&&this.PrivateAct && this.PrivateAct(this);
            ...
            for (let i = 3; i >= 0; i--) {
              for (let j = 1; j <= C; j++) {
                ...
                p && (p.canEat)&&(p.Stature>=0)&& (p.EName != "oBrains"&& p.EName != "oCherryBomb" &&
                  p.EName != "oJalapeno" && p.EName != "oDoomShroom" && p.EName != "oSunFlower") && (p.AttackedLX < $(d).offsetLeft) && (p.AttackedRX > $(d).offsetLeft) && (PlayAudio("splat1"), (p.HP -= 20), ($(d)&& ClearChild($(d))));
            ...
            var a = $Z[c];
                    $Z[c] && (oSym.addTask(0,
                        function(f) {
                            var e = $Z[f],
                                d;
                            e && (d = NewImg("", "images/interface/blank.png", "width:306px;height:300px;left:" + (e.X - 16) + "px;top:" + (e.pixelTop - 90) + "px;z-index:20"), d.src = "images/Zombies/JackinTheBoxZombie/Boom.gif",PlayAudio("explosion"),EDPZ.appendChild(d), oSym.addTask(70, ClearChild, [d]), e.PZ ? ((function(k, g) {
	...
	return InheritO(CZombies, {
		...
		gethPea: function(e, b, c) {
		    ...
		    getPoison: function(e,b,c){
				var a;
				T=oSym.Now+(e.Pnum*200);
				e.Pnum<5&&(e.Pnum+=1);
                e.FreePoisonTime=a=T;
...
oDancingZombie= InheritO(OrnNoneZombies, {
  ...
  ChkActs1: function(e, b, f, a) {
	...
	ChkActs: function(g, d, h, c) {
		...
		if(g.HPlook&&!g.bHP){
	g.bHP=1;
    var E= NewEle("daHP", "div", "position:absolute;color:#fff;top:" + (g.pixelTop + 100) + "px;left:" + (g.ZX+60) + "px;width:100%;font-size:12px;z-index:50", "", EDAll);
...
oDancingZombie2= InheritO(oDancingZombie, {
        ...
        g && g.beAttacked && (e.src = "images/Zombies/DancingZombie/Summon2.gif", oSym.addTask(10,
          function(t, s, x) {
            var h = $Z[t],
              ...
              u = [],
              o = 0,
              ZP = h.hard==2?[new oDancingZombie,new oBucketheadZombie,new oNewspaperZombie,new oFlagZombie,new oFootballZombie1,new oJackinTheBoxZombie,new oPeaZombie,new oDancingZombie1,new oDancingZombie2,new oPoleVaultingZombie1,new oPoleVaultingZombie2,new oNewspaperZombie3]:
				[new oZombie, new oZombie2, new oZombie3, new oDancingZombie, new oConeheadZombie, new oBucketheadZombie, new oNewspaperZombie, new oFlagZombie, new oScreenDoorZombie, new oFootballZombie,new oJackinTheBoxZombie,new oPeaZombie,new oDancingZombie1,new oDancingZombie2,new oJalapenoZombie, new oPoleVaultingZombie,new oPoleVaultingZombie1,new oPoleVaultingZombie2, new oNewspaperZombie3],
              q,
              l;
            if (h && h.beAttacked) {
              s.src = "images/Zombies/DancingZombie/Summon3.gif";
              while (r--) {
                (q = m[r]) && (!(l = q[0]) || !$Z[l]) && (u[o] = (w[o] = ZP[Math.floor(Math.random() * ZP.length)]).CustomBirth(q[1], q[2](v), 100, q[0] = "Z_" + Math.random()), n.push(NewImg("", k, "z-index:" + q[3] + ";left:" + q[4](v) + "px;top:" + q[5] + "px", EDPZ)), ++o)
              }
              oSym.addTask(220,
                function() {
                  var i = arguments.length;
...
oZombie2 = InheritO(oZombie, {
	...
	if(a.PZ){
                for(let i = 3;i >= 0;i--){
                    for(let j = 1;j <= C;j++){
                        let p = oGd.$[a.R+"_"+j+"_"+i];
                       a&&(a.beAttacked)&&p&& (p.canEat)&&(p.Stature>=0) && (p.EName != "oBrains" && p.EName != "oCherryBomb"&& p.EName != "oJalapeno" && p.EName != "oDoomShroom"&& p.EName != "oSunFlower") &&
						   (p.AttackedLX < $(d).offsetLeft) && (p.AttackedRX > $(d).offsetLeft)&& (PlayAudio("potato_mine"),((a.hard==2)?CustomZombie(oBoom,p.R,p.C):(p.HP -=1000)),($(d) && ClearChild($(d))));
                        p && (p.canEat) && (p.HP <= 0) && p.Die();
                    }
                }
	}
...
oConeheadZombie1= InheritO(OrnIZombies, {
	...
	var num=Math.random()*100,
      ...
      C = Math.max(Math.min(GetC(c.ZX),9),1);
...
PrivateAct2:function(a){
	...
	PrivateAct1:function(a){
            if(!a.bool){
                a.Speed =a.OSpeed=4.8;
                var C = GetC(a.ZX-1);
for (i=0;i<4;i++){
	for (let l= (a.R - 1 >= 1 ? a.R - 1 : 1); l<= (a.R + 1 <= oS.R ? a.R + 1 : oS.R); l++) {
                var p = oGd.$[(a.hard==2?l:a.R)+"_"+C+"_"+i];
                if(a.beAttacked&&p && p.canEat && (p.EName != "oPotatoMine" && p.EName != "oCherryBomb" && p.EName != "oJalapeno" && p.EName != "oDoomShroom")&&a.PZ){
...
oBucketheadZombie= InheritO(oConeheadZombie,{
	...
	var ZP=[oZombie,oZombie2,oZombie3,oDancingZombie,oDancingZombie1,oConeheadZombie,oBucketheadZombie,oZomboni,oNewspaperZombie,oNewspaperZombie2,oNewspaperZombie3,oFlagZombie,oScreenDoorZombie,oFootballZombie,oBalloonZombie,oPeaZombie,oJackinTheBoxZombie,oDiggerZombie];
            ...
            oSym.addTask(50, function(f, e) {
                var h = $Z[f],
                    g;
                h && h.beAttacked && !h.FreeFreezeTime && !h.FreeSetbodyTime && ((g = $P[e])&&g.getHurt(h, 2, h.Attack),(g&&g!==undefined)&&(h.hard==2?CustomZombie(ZP[Math.floor(Math.random() *ZP.length)],g.R,g.C):
			CustomZombie(oZombie3,g.R,g.C)),h.JudgeAttack())
            }, [d, c]);
            this.PrivateAttack && this.PrivateAttack(this)
	},
	PrivateBirth: function(a){
	    a.PrivateAct = Math.round(Math.random()*100)>15 ?(function(){}): a.PrivateAct2;
	},
  PrivateAct2: function(a) {
    if (!a.A) {
      a.A = 1;
	...
	a.PZ!==e&&EditImg($(z),0,"images/Plants/PumpkinHead/pumpkin_damage1.gif",{transform:a.PZ?"rotateY(0deg)":"rotateY(180deg)"},0);
      ...
      oSym.addTask(500,function(a) {
        for (let i = (a.R - 1 >= 1 ? a.R - 1 : 1); i <= (a.R + 1 <= oS.R ? a.R + 1 : oS.R); i++) {
            ...
            for (w = 0; w < A.length; w++) {
              var t = A[w],
                s;
              if ((t.CName == "铁桶僵尸" || t.CName == "领带僵尸" || t.CName == "路障僵尸" || t.CName == "铁栅门僵尸"||t.CName == "辣椒僵尸"||t.CName == "寒冰射手僵尸"||t.CName == "鸭子救生圈僵尸"||t.CName=="路障鸭子救生圈僵尸"||t.CName== "铁桶鸭子救生圈僵尸")&&
...
oFootballZombie= function() {
    ...
    return InheritO(oConeheadZombie,{
	...
	bedevil: function(c) {
		...
		c.WalkDirection = 1;
	    c.ZX = c.AttackedRX;
		c.ChkActs = CZombies.prototype.ChkActs1;
		oP.MonPrgs()
		},
	PicArr: (function() {
...
ChkActs: function(h, f, j, e) {
    ...
    h.hard!==2&&(h.ZX <= 250) && (h.ChkActs = h.ChkActs1,
      h.ExchangeLR(h, 1),
      h.ZX = h.AttackedRX,
      h.WalkDirection = 1);
		if(h.HPlook&&!h.bHP){
...
oFootballZombie1= InheritO(oConeheadZombie, {
  ...
  PrivateAct: function(a) {
    if (!a.bool) {
      ...
      var z = $(a.id);
      z.SquashHeadId = "Squash" + Math.random();
      var squash = NewImg(z.SquashHeadId, "images/Plants/GatlingPea/GatlingPea.gif", "position:absolute;transform:"+(a.PZ?"rotateY(180deg);":"rotateY(0deg);")+"left:40px;top:15px;",0);
      z.appendChild(squash);
	oSym.addTask(10,function(a,Ja,z,e){
	a.PZ!==e&&EditImg($(z),0,"images/Plants/PumpkinHead/pumpkin_damage1.gif",{transform:a.PZ?"rotateY(180deg)":"rotateY(0deg)"},0);
	oSym.addTask(10,arguments.callee,[a,squash,z.SquashHeadId,e])
	},[a,squash,z.SquashHeadId,a.PZ]);
	}
	var p=$(a.id);
	...
	for (let i = (a.R - 1 >= 1 ? a.R - 1 : 1); i <= (a.R + 1 <= oS.R ? a.R + 1 : oS.R); i++) {
      ...
      oSym.addTask(1, function(a) {
        ...
        oSym.addTask(1, function(z, d, a, y,num) {
            ...
            for (let i = 3; i >= 0; i--) {
              for (let j = 1; j <= C; j++) {
                let p = oGd.$[(num>25?a.R:GetR($(d).offsetTop+30)) + "_" + j + "_" + i];//若子弹偏转角度过小则直接判定为本行
	...
	Produce: '撑杆僵尸可以跳N格，跳跃完毕后变成巨人攻击方式<p>韧性：<font color="#FF0000">中(800)</font><Br>速度：<font color="#FF0000">快,而后慢(跳跃后)</font><BR>特点：<font color="#FF0000">跃过他所碰到的第一筑植物</font></p>一些僵尸渴望走得更远、得到更多，这也促使他们由普通成为非凡。那就是撑杆僵尸。',
		...
		AttackZombie: function(d, c) {
			...
			function(f, e) {
				var h = $Z[f],
				g;
				h && h.beAttacked && !h.FreeFreezeTime && !h.FreeSetbodyTime && ((g = $Z[e]) && g.getHit0(g, 1500, 0), h.JudgeAttack())
			},
			[d, c])
		},
		AttackZombie2: function(e, d, c) {
...
if(!a.gif){
      z.PeaHead = "Pea" + Math.random();
      let pea = NewImg(z.PeaHead,a.PicArr[15],"position:absolute;width:122px;height:157px;transform:rotateY(180deg);left:150px;top:-20px;", 0);
      z.appendChild(pea);
	a.gif=1;
    ...
    if (!a.bool&&(a.Polenum==1)) {
      ...
      oSym.addTask(20, function(a) {
        ...
        a.PZ &&a.Polenum&& (a.HP > 60) && oSym.addTask(20, arguments.callee, [a]);
      }, [a]);
    }
},
bedevilAct:function(){
...
if(!a.g){
      z.PeaHead = "Pea" + Math.random();
      let pea = NewImg(z.PeaHead,a.PicArr[15], "position:absolute;width:122px;height:157px;left:70px;top:-20px;", 0);
      z.appendChild(pea);
	a.g=1;
    ...
    if (!a.ec) {
      ...
      EditImg($(z.PeaHead),0,a.PicArr[14],{},0);
            ...
            Z&& (Z.Altitude == 1) && ((Z.getHit0(Z, 6, 0)));
                if ($(d).offsetLeft>=oS.W) {
                  ClearChild($(d));
                  $(d).isDie = true;
                }!($(d).isDie) && oSym.addTask(1, arguments.callee, [z, d, a]);
				oSym.addTask(30,function(a,z){a&&a.beAttacked&&EditImg($(z.PeaHead),0,a.PicArr[15],{},0)},[a,z])
              } catch (e) {}
            }, [z, d, a]);
...
try{
        ...
        oSym.addTask(50, function(s,Z) {
			var g = oZ.getArZ(Z.ZX, Z.ZX+100,a.R),
        h = g.length;
  ...
  oNewspaperZombie1= InheritO(oNewspaperZombie, {
	    ...
	    if(!$Z[a.id]){return}
			...
			if(a.PZ){
                for(let i = 3;i >= 0;i--){
                    for(let j = 1;j <= C;j++){
                        let p = oGd.$[a.R+"_"+j+"_"+i];
                        p && (p.canEat) &&(p.Stature>=0)&& (p.EName != "oBrains" && p.EName != "oCherryBomb" && p.EName != "oJalapeno" && p.EName != "oDoomShroom"&& p.EName != "oSunFlower") && (p.AttackedLX < $(d).offsetLeft) && (p.AttackedRX > $(d).offsetLeft)&& 
...
getHit0:function(a,c){
	...
	let p = oZ.getHZ1($(d).offsetLeft + 50,a.R);
...
SunNum:2000,
  ...
  ChkActs: function(h, f, j, e) {
    var d, c, g;
    h.Ornaments !== 0 && (h.ZX <= 800) && (h.Speed = h.oSpeed = 0);
...
if(d.HPlook&&!d.bHP){
	d.bHP=1;
    var b = NewEle("dHP", "div", "position:absolute;color:#fff;top:" + (d.pixelTop + 100) + "px;left:" + (d.ZX) + "px;width:100%;font-size:12px;z-index:50", "", EDAll);
    var A = "hp" + Math.random();
    dHP.id = A;
    var C = $(A);
...
NormalAttack: function(c, b) {
    ...
    if(!a.bool&&(!oS.CannotShoot)){
		...
		var z = $(a.id);
        z.PeaHead = "Pea" + Math.random();
        var Pea = NewImg(z.PeaHead,"images/Plants/SnowPea/SnowPea.gif","position:absolute;width:80px;height:80px;transform:"+(a.PZ?"rotateY(180deg);":"rotateY(0deg);")+"left:30px;top:15px;",0);
        z.appendChild(Pea);
	...
	a.PZ!==e&&(EditImg($(FumeDoor),0,"images/Plants/FumeShroom/FumeShroom.gif",{transform:a.PZ?"rotateY(180deg)":"rotateY(0deg)"},0),
			   EditImg($(PeaHead),0,"images/Plants/SnowPea/SnowPea.gif",{transform:a.PZ?"rotateY(180deg)":"rotateY(0deg)"},0));
...
oSym.addTask(300,function(a){
	if(!$Z[a.id]&&!a.beAttacked){return}
			...
			if(a.PZ){
                ...
                p && (p.canEat)&&(p.Stature>=0)&& (p.EName != "oBrains"&& p.EName != "oCherryBomb" && 
				    ...
				    p.EName != "oTorchwood") &&(p.AttackedLX < $(d).offsetLeft) && (p.AttackedRX > $(d).offsetLeft) && (PlayAudio("splat1"),(p.HP -= 50),(p.NormalAttack=function(){}),($(p.id).style.opacity = 0.5),($(d) && ClearChild($(d))));
		p && (p.canEat)&&(p.Stature>=0)&& (p.EName != "oBrains"&& p.EName != "oCherryBomb" && p.EName != "oJalapeno" && p.EName != "oDoomShroom"&& p.EName != "oSunFlower"&& p.EName != "oIceShroom") && (p.AttackedLX < $(d).offsetLeft) && (p.AttackedRX > $(d).offsetLeft)&& (PlayAudio("splat1"),(p.HP -= 20),($(d) && ClearChild($(d))));
                p && (p.canEat) && (p.HP <= 0) && p.Die();
                    }
                }
			}
	...
	a&&(!a.Ornaments)&&ClearChild($(P.FumeDoor));	
