...
oGloomShroom = InheritO(oFumeShroom, {
	...
	NormalAttack: function() {
		...
		for (g = k.MinR; g <= f; g++){
			e = oZ.getArZ(c, b, g);
			for (h = e.length; h--; (a = e[h]).Altitude < 2 && a.getHit1(a, 100)
			){}
		}
		oSym.addTask(100,
		function(i) {
...
oHypnoShroom = InheritO(oFumeShroom, {
	...
	InitTrigger: function() {},
	PrivateBirth:function(b){
         asyncInnerHTML((a=Math.round(Math.random()*100)>20 ? new oFootballZombie:Math.round(Math.random()*100)>25 ? new oNewspaperZombie: new oNewspaperZombie3).CustomBirth(b.R,b.C,0,"auto"), function(n, m) {
             EDPZ.appendChild(n);
             m.Birth();
             m.bedevil(m);
             //document.write()
...
oCactus = InheritO(CPlants, {
	...
	NormalAttack: function() {
		...
		function(g, i, d, k, h, l) {
			var j, f = GetC(k),
			e = oZ["getZ" + d](k, h);
			e && e.Altitude == 1 ? (Math.round(Math.random()*100)>4?e.getPea(e,10,d):e.getrPea(e,20,d),oSym.addTask(25,arguments.callee,[g, i, d, k, h, l])): (k += (j = !d ? 5 : -5)) < oS.W && k > 100 ? (i.style.left = (l += j) + "px", oSym.addTask(1, arguments.callee, [g, i, d, k, h, l])) : ClearChild(i)
		},
