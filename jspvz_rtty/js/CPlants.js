...
oSym.addTask(1000,function(s,a){
	...
	NormalAttack: function() {
		...
		function(f, j, h, c, n, i, m, k, o, g) {
			...
			A&&(A.Vasenum+=1,A.Vasenum>=50&&(A&&A.setVase())));
			d && d.Altitude == 1 ? (d[{
				"-1": "getSnowPea",
				0 : "getPea",
				1 : "getFirePea"
...
oSnowPea = InheritO(oPeashooter, {
	...
	NormalAttack: function() {
		...
		A&&(A.Vasenum+=2,A.Vasenum>=50&&(A&&A.setVase())));
			d && d.Altitude == 1 ? (d[{
				"-1": "getSlowPea1",
				0 : "getSlowPea",
				1 : "getSlowPea"
...
NormalAttack1:function(a) {
	...
	NormalAttack2: function() {
		...
		a = function(j, i, h) {
			...
			oSym.addTask(1,
			function(m, k, l, i, j) {
				j(oZ.getZ0(m, k), 0, i) && ((m += 5) >oS.W ? ClearChild(i) : (i.style.left = (l += 5) + "px", oSym.addTask(1, arguments.callee, [m, k, l, i, j])))
			},
...
oScaredyShroom = InheritO(oFumeShroom, {
	...
	NormalAttack: function() {
		...
		function(g, e) {
			...
			f && SetVisible(f);
			$P[a]&&($P[a].AttTime-=2);
			oSym.addTask(130-Math.min(80,c.AttTime),
			function(h) {
				var i = $P[h];
				i && (i.Attacking = 0)
			},
