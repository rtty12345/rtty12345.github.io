(() => {
	...
	oS.Init({
		...
		StartGameMusic: "True_Admin", SummonCoolTime: 55, // 生成卡片的间隔
		LargeWaveFlag: {
		...
		StartGame: function() {
			...
			oSym.addTask(100, function(NowFlag) { // 夜晚墓碑
				if (oP.FlagZombies >= NowFlag) 	oP.SetTimeoutTomZombie(oS.StageZCard_2), AppearTombstones(4, 9, 1, (R) => R != 3 && R != 4), NowFlag += 3;
				...
				oP.Monitor({
					f: function() {
						...
						(function() { // 卡片移动，要适应不同的倍速
							let b = ArCard.length, a, c, d, SPX = 1 * (oSym.NowStep / 1), STk = oSym.NowStep;
							while (b--) a = ArCard[b], c = a.PixelTop, d = 60 * b, $(a.DID).style.top = (a.PixelTop = (c > 60 * b) ? (c - SPX) : (d)) + "px";
							oSym.addTask(STk, arguments.callee, [])
						})();
					}, ar: []
				});
				oSym.addTask(700, () => { oP.AddZombiesFlag(), SetVisible($("dFlagMeterContent")); }, []);
			})
		}
	}, {
		...
		GetRandom: (R) => Math["floor"](Math["random"]() * R) == 0,
